...
KQL is the declarative query language used to request projections of KOS state. It is not an imperative command language. A KQL expression describes the desired view of the system; the Runtime resolves the query and the Renderer presents the resulting projection.
...
- **View qualifiers** (optional): presentation or expansion refinement.
...
defines the standard task-state projection.
...
KQL queries are declarative projections. They do not execute by themselves; they request a view from the Runtime State.
...
A query may combine both, but the final segment should remain a view qualifier or a bounded subject.
...
- Prefer descriptive operator-facing labels in Spanish.
